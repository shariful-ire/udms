  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
      <Field label="Name" error={errors.name?.message} required>
        <input {...register("name")} placeholder="e.g. Rice purchase, Gas bill…"
          className={inputCls(errors.name)} />
      </Field>

      <div className="grid grid-cols-2 gap-4">
        <Field label="Date" error={errors.expense_date?.message} required>
          <input type="date" {...register("expense_date")}
            className={`${inputCls(errors.expense_date)} [color-scheme:light] dark:[color-scheme:dark]`} />
        </Field>
...
export function RequestForm({ onSubmit, isLoading }) {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm({
    ...
    defaultValues: { date: today, meal_type: "", reason: "" },
  });

  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
      <div className="grid grid-cols-2 gap-4">
        <Field label="Meal Date" error={errors.date?.message} required>
          <input type="date" {...register("date")}
            min={today}
            className={`${inputCls(errors.date)} [color-scheme:light] dark:[color-scheme:dark]`} />
        </Field>
      ...
      <Field label="Reason (optional)" error={errors.reason?.message}>
        <textarea {...register("reason")} rows={3} placeholder="Why do you need this meal?"
